...
description: 'クレカ前払い前提の崩れに伴う仕様再編（自動キャンセル停止・請求書抑制縮小・金額確定フロー・前日自動請求）— 全て実装済み（2026-07-24）'
sidebar:
  order: 55
---

> **💡 運用者向け:** 2026-07 に「クレカ＝前払い強制」の旧前提が崩れ、**後払いクレカが基本**になりました。これに伴う仕様変更4本は**全て実装済み**です。(1) 開始日基準の自動キャンセルを**停止**（スタッフ手動管理へ ✅）(2) クレカ伝票の**商品ロック**（金額確定時・✅）(3) 後払いクレカの**金額確定フロー**（全額一本請求・配送料の個別決済廃止 ✅）(4) **前日自動請求**（店頭受取の後払いクレカ・レンタル開始前に自動で請求書発行 ✅）。技術詳細は開発者モード（ページ右上トグル）ON でご確認ください。
...
| **#2441** | 続編（7/23 A項） | 前日自動請求 cron（pickup 後払いクレカ・レンタル開始前に自動請求書発行） | ✅ **実装済**（commit `defd224c`〜`64221a1d`・2026-07-24）|
