---
title: '自動キャンセルの仕組み'
description: '未決済予約の自動キャンセル条件・対象外・タイミング・2つの cron（開始日基準の③は#2407で停止運用・銀行振込確定予約・後払い・管理画面仮予約は保護）'
sidebar:
  order: 50
tableOfContents: false
---

> **💡 運用者向け:** 未決済のまま放置された予約（カート経由）は、在庫ロックを防ぐため自動キャンセルされます（主に135分間のカート放置回収）。**開始日基準の自動キャンセル（毎日02:00）は #2407（2026-07-20）で停止し、スタッフの手動管理に切り替わりました**。ただし**銀行振込の確定予約・後払い（返却後決済）伝票・管理画面で手動作成した仮予約は従来通り自動キャンセル対象外**（入金や返却を待つため）です。キャンセル条件・タイミング・ジョブ・トラブルシューティングの技術詳細は開発者モード（ページ右上のトグル）を ON にしてご確認ください。

:::dev
> **出典:** BE #1772 調査結果（2026-05-23）+ #1944（銀行振込保護）+ #2072/#2073（仮予約自動失効 cron 廃止）+ #2284/#2330（deferred-checkout 除外）+ **#2407（開始日基準 auto-cancel + 7日前 final reminder 停止・2026-07-20）** — 2026-07-20 时点
...
| cron | 頻度 | 役割 | 主な対象 | 現状 |
|------|------|------|---------|------|
| **`draft-order-cleanup`**（①）| 5分毎（既定135分で失効）| 未決済 Draft Order の短期 hold 解放（カート落ち）| 未決済の cart 経由予約 | **稼働中** |
| **`draft-booking-lifecycle`**（③）| 毎日 02:00 JST | 開始日基準の支払期限失効（開始6日前）| 開始日が近づいた未決済予約 | **#2407 で本番停止**（env `DRAFT_AUTO_CANCEL_DAYS_BEFORE=0`・スタッフ手動管理へ）|

> **#2407（2026-07-20）重要**: ③（開始日基準 auto-cancel・Phase3）とその前段の7日前 final reminder（Phase2「明日自動キャンセルされます」）は、後払い基本化に伴い**本番で停止**しました（env `DRAFT_AUTO_CANCEL_DAYS_BEFORE=0`・Phase2/Phase3 gate・スタッフ手動キャンセル運用）。現状稼働している自動キャンセルは **①（135分・放置カート回収）のみ**。督促リマインダーは Phase1（8日前 standard reminder・全顧客・auto-cancel 言及なし）が担保します。③はコード上残存（env で再開可能・`draft-booking-lifecycle.service.ts` の `sendFinalReminders` / `autoCancelExpiredDraftBookings` gate）。
...
>
> **#2407（2026-07-20）以降**: 上記のクレカ伝票キャンセル（B260522012/B260521017/B260521013）は③（`draft-booking-lifecycle`・開始日基準）が稼働していた時期（2026-05）の実証データです。③は #2407 で本番停止したため、現在は③による開始日基準のキャンセルは発生しません。稼働中の自動キャンセルは①（135分・放置カート回収）のみです。
