...
### 伝票作成通知（店側）（sendShopBookingCreatedNotification・#2698 / S138 拡張）

- **件名**: `【EASE Rental】予約が登録されました（${displayId}）`（`displayId` = 伝票番号・未採番なら bookingId）
- **FROM**: `rental-info@iziz.co.jp`（送信専用）
- **email_type**: `shop_booking_created`
- **トリガー**: **①FE 管理画面での伝票作成時（#2698）＋ ②カート依頼の受領時（S138・2026-08-26 拡張）**。いずれも Shopify Order が作られない（①）／依頼時点では Shopify 注文通知が飛ばない（②）ため、このメールが店側への受領通知を担う。決済完了（Order化）時に Shopify から「注文番号#…が注文されました」が別途届くため二重にならない（依頼時=BE、決済完了時=Shopify の棲み分け）。webhook 経由（Shopify Order 由来）は除外 — 「注文されました」通知が既に飛んでいるため

**文面①管理画面作成（source=backend_app）:**
```
FE管理画面から伝票が作成されました。
...
**文面②カート依頼（source=cart）— 1〜2行目のみ差し替え、以降は共通:**
```
カートからご予約の依頼を受け付けました。
...
- **トリガー**: お問い合わせフォーム送信時（App Proxy 受付・Shopify ストアの contact ページ）。#2690 B070 で全 `form_type` 共通の1文面に簡素化（在庫確認/一般の文面分岐は廃止・`form_type` 振分自体は #2360 のままスタッフ通知側で機能）

**宛名（salutation）:**
- 法人（companyName あり）: `${companyName} ご担当者様`
- 個人（companyName なし）: `${contactName} 様`

**本文（#2690 B070: シンプル受付通知・問い合わせ内容のエコーバックなし）:**
```
${salutation}
...
**変数**: `${salutation}`（= `${companyName}` / `${contactName}` の法人/個人分岐のみ）

:::note[#2690 B070 でエコーバック廃止]
旧 iziz サイト互換の全項目エコーバック（お客様情報/お問合せ内容/支払方法/搬入出/在庫確認商品/合計金額）と在庫確認/一般の greeting 分岐、旧呼称「EASE WEBカタログ」は #2690 B070（2026-08-18・ユーザー裁定）で廃止。在庫確認はストア側で直接確認可能になったため。金額記載が無くなったため #2216「（税込）」統一の対象外という事情も解消。`COMPANY_SIGNATURE` を本文内に直接配置する点（#2342・`withSignature()` 経由でない）は従来どおり。
:::
...
- **トリガー**: ドラフトオーダーから請求メール送信時。送信経路は ①予約作成時（**即時支払い pay_now=true の顧客のみ**・銀行振込/後払いは除外）②**レンタル開始前日 18:00 cron**（後払いクレカ・店頭受取 #2441 — `draft_order_rebuild {sendShopifyInvoice: true}` 経由で BE 請求書PDFメールと2通）③再送・再発行EP
- **件名**（2026-08-26 変更・BE が `draftOrderInvoiceSend` に subject を指定）:

| 顧客 | 件名 |
|------|------|
| 後払いクレカ（金額確定後の送信） | `【EASE】明細書送付とカード決済のお願い（伝票番号: ${B伝票番号}）` |
| 前払いクレカ pay_now（予約作成直後） | `【EASE】ご予約内容とカード決済のお願い（伝票番号: ${B伝票番号}）` |
| 銀行振込等（非クレカ契約） | Shopify テンプレート標準の `請求書 {{name}}` のまま（`{{name}}` = #D… ドラフトオーダー番号） |

  変更理由: 旧来は全員 `請求書 {{name}}` で、`{{name}}` がドラフトオーダー番号（#D…）のため顧客が伝票番号（B…）と紐付けられず、「請求書」という文言も実態（明細＋カード決済リンク）とズレていたため。**銀行振込顧客には「カード決済」文言を絶対に送らない**ため、非クレカ契約はテンプレート標準のまま（件名判定は契約 `payment_method` ベース）。本文は全員 Shopify テンプレートのまま（件名のみ差し替え）。
- **冒頭文案**: タイトル「ご予約手続きを完了する」/ 本文 `ご予約内容をお確かめのうえ、お早めにご決済をお願いいたします。` / ボタン「ご予約を確定する」（`{{ invoice_url }}` = Shopify checkout）。旧文案「60分以内にご決済」は仮予約の在庫ホールド60分由来でしたが、後払いクレカ基本化で大多数の受信タイミングが前夜18時となり実態と乖離、#2692 で自動キャンセル系撤去により期限ペナルティも不存在のため期限を書かない文案に変更
- **マイアカウント案内**（2026-08-26 追加・管理画面＋テーマ正本 `1febc9e` に反映済み）: 決済ボタン直下に1行 — `※ご予約内容・レンタル伝票（PDF）は マイアカウント でもご確認・ダウンロードいただけます。`（リンク先 `{{ shop.url }}/account`）。PDF添付の代替導線。前払い（予約直後）・後払い（金額確定後）の両タイミングで成立する文言（「明細書」等のタイミング限定語は回避）
...
- **最終確認日**: 2026-08-26
- **対象 BE commit**: `8ce0f2ec` 〜 `929187dd`（main HEAD）。2026-08-26 にメール関連差分を反映:
  - **DO① 決済リンクメール件名（`d67c8b91`）**: Shopify「下書き注文の請求書」の件名を、クレカ契約の伝票のみ BE 指定に変更（後払い=「明細書送付とカード決済のお願い」/ 前払い=「ご予約内容とカード決済のお願い」+ 伝票番号）。非クレカ契約はテンプレート標準「請求書 {{name}}」のまま（実装: `bookings/utils/do1-invoice-email-subject.ts`）
  - **マイアカウント案内（テーマ正本 `1febc9e`）**: 下書き注文の請求書メールの決済ボタン直下にレンタル伝票PDFのDL導線を1行追加（`{{ shop.url }}/account`）
  - 2026-08-24 実施分:
